...
public unsafe struct XMMatrix : IEquatable<XMMatrix>
{
    ...
    /// <param name="rows">The rows.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public XMMatrix(ReadOnlySpan<XMVector> rows)
    {
        if (rows.Length != 4)
        {
            throw new ArgumentOutOfRangeException(nameof(rows));
    ...
    /// <param name="array">The values.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public XMMatrix(ReadOnlySpan<float> array)
    {
        if (array.Length != 16)
        {
            throw new ArgumentOutOfRangeException(nameof(array));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [SuppressMessage("Usage", "CA2225:Les surcharges d'opérateur offrent d'autres méthodes nommées", Justification = "Reviewed.")]
    public static implicit operator ReadOnlySpan<float>(XMMatrix value)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    [SuppressMessage("Usage", "CA2225:Les surcharges d'opérateur offrent d'autres méthodes nommées", Justification = "Reviewed.")]
    public static implicit operator Span<float>(XMMatrix value)
    {
        return value.AsSpan();
    }
    ...
    public ReadOnlySpan<float> AsReadOnlySpan()
    {
        fixed (void* ptr = &this)
        {
            return new ReadOnlySpan<float>(ptr, 16);
        }
    }
    ...
    {
        return obj is XMMatrix matrix && Equals(matrix);
    }
    ...
    /// <returns></returns>
    public readonly bool Equals(XMMatrix other)
    {
