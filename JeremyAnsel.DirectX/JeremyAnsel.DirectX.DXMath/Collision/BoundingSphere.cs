...
public unsafe struct BoundingSphere : IEquatable<BoundingSphere>
{
    ...
    /// <returns>The new <see cref="BoundingSphere"/> containing the specified points.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static BoundingSphere CreateFromPoints(ReadOnlySpan<XMFloat3> points)
    {
        if (points.Length == 0)
        {
    ...
    public PlaneIntersectionType Intersects(XMVector plane)
    {
        ...
        Internal.FastIntersectSpherePlane(v_center, v_radius, plane, out XMVector outside, out XMVector inside);
        ...
        XMVector plane5)
    {
        ...
        Internal.FastIntersectSpherePlane(v_center, v_radius, plane0, out XMVector outside, out XMVector inside);
