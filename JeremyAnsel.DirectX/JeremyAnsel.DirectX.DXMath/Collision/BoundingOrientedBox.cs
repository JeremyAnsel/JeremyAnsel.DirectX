...
public unsafe struct BoundingOrientedBox : IEquatable<BoundingOrientedBox>
{
    ...
    /// <returns>The new <see cref="BoundingOrientedBox"/> containing the specified points.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static BoundingOrientedBox CreateFromPoints(ReadOnlySpan<XMFloat3> points)
    {
        ////-----------------------------------------------------------------------------
    ...
    public ContainmentType Contains(BoundingFrustum fr)
    {
        ...
        Span<XMFloat3> corners = stackalloc XMFloat3[BoundingFrustum.CornerCount];
    ...
    {
        return obj is BoundingOrientedBox box && Equals(box);
    }
    ...
    /// <returns></returns>
    public readonly bool Equals(BoundingOrientedBox other)
    {
