...
public unsafe class DxgiSwapChain : DxgiDeviceSubObject
{
    ...
    /// The interface GUID.
    /// </summary>
    public static readonly Guid DxgiSwapChainGuid = typeof(IDxgiSwapChain).GUID;
    ...
    /// </summary>
    public DxgiSwapChain(nint comPtr)
        : base(comPtr)
    {
        _comPtr = comPtr;
    ...
    {
        nint ptr = GetBuffer(buffer, DxgiSurface.DxgiSurfaceGuid);
    ...
    public bool GetFullscreenState(out DxgiOutput? target)
    {
        ...
        int hr = _comImpl->GetFullscreenState(_comPtr, &fullscreen, &ptr);
        Marshal.ThrowExceptionForHR(hr);
        target = ptr == 0 ? null : new DxgiOutput(ptr);
        return fullscreen != 0;
    }
