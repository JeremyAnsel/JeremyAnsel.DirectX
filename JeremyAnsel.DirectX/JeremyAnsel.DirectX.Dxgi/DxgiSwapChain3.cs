...
/// <summary>
/// Extends <c>IDXGISwapChain2</c> with methods to support swap back buffer scaling and lower-latency swap chains.
/// </summary>
[SecurityCritical, SuppressUnmanagedCodeSecurity]
[SkipLocalsInit]
public unsafe class DxgiSwapChain3 : DxgiDeviceSubObject
{
    /// <summary>
    /// The interface GUID.
    /// </summary>
    public static readonly Guid DxgiSwapChain3Guid = typeof(IDxgiSwapChain2).GUID;

    private readonly nint _comPtr;
    private readonly IDxgiSwapChain2* _comImpl;

    /// <summary>
    /// Initializes a new instance of the <see cref="DxgiSwapChain2"/> class.
    /// </summary>
    public DxgiSwapChain3(nint comPtr)
        : base(comPtr)
    {
        _comPtr = comPtr;
        _comImpl = *(IDxgiSwapChain2**)comPtr;
    }
    ...
    public DxgiMatrix3x2F MatrixTransform
    {
        get
        {
            int size = DxgiMatrix3x2F.NativeRequiredSize();
            byte* ptr = stackalloc byte[size];
            int hr = _comImpl->GetMatrixTransform(_comPtr, ptr);
            Marshal.ThrowExceptionForHR(hr);
            return DxgiMatrix3x2F.NativeReadFrom((nint)ptr);
        }
        ...
        set
        {
            int size = DxgiMatrix3x2F.NativeRequiredSize();
            byte* ptr = stackalloc byte[size];
            DxgiMatrix3x2F.NativeWriteTo((nint)ptr, value);
            int hr = _comImpl->SetMatrixTransform(_comPtr, ptr);
            Marshal.ThrowExceptionForHR(hr);
        }
    }
    ...
    {
        nint ptr = GetBuffer(buffer, DxgiSurface3.DxgiSurface3Guid);
        return new DxgiSurface3(ptr);
    }
    ...
    public bool GetFullscreenState(out DxgiOutput3? target)
    {
        ...
        else
        {
            try
            {
                target = new DxgiOutput3(DXUtils.QueryInterface(ptr, DxgiOutput3.DxgiOutput3Guid));
            }
            finally
            {
                DXUtils.Release(ptr);
    ...
    public DxgiOutput3 GetContainingOutput()
    {
        ...
        try
        {
            return new DxgiOutput3(DXUtils.QueryInterface(ptr, DxgiOutput3.DxgiOutput3Guid));
        }
        finally
        {
            DXUtils.Release(ptr);
    ...
    {
        int hr = _comImpl->SetSourceSize(_comPtr, width, height);
        Marshal.ThrowExceptionForHR(hr);
    }
    ...
    public void GetSourceSize(out uint width, out uint height)
    {
        uint w;
        uint h;
        int hr = _comImpl->GetSourceSize(_comPtr, &w, &h);
        Marshal.ThrowExceptionForHR(hr);
        width = w;
        height = h;
    }
