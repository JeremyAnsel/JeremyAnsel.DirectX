...
[SkipLocalsInit]
public unsafe class DxgiFactory1 : DxgiObject
{
    /// <summary>
    /// The interface GUID.
    /// </summary>
    public static readonly Guid DxgiFactory1Guid = typeof(IDxgiFactory1).GUID;

    private readonly nint _comPtr;
    private readonly IDxgiFactory1* _comImpl;

    /// <summary>
    /// Initializes a new instance of the <see cref="DxgiFactory1"/> class.
    /// </summary>
    public DxgiFactory1(nint comPtr)
        : base(comPtr)
    {
        _comPtr = comPtr;
        _comImpl = *(IDxgiFactory1**)comPtr;
    }
    ...
    {
        nint ptr;
        int hr = NativeMethods.CreateDxgiFactory(typeof(IDxgiFactory1).GUID, &ptr);
        Marshal.ThrowExceptionForHR(hr);
        return new DxgiFactory1(ptr);
    }
    ...
    public DxgiAdapter1? EnumAdapters(uint i)
    {
        nint ptr;
        int hr = _comImpl->EnumAdapters1(_comPtr, i, &ptr);
        return hr != 0 ? null : new DxgiAdapter1(ptr);
    }
    ...
    public DxgiAdapter1 GetAdapter(uint i)
    {
        nint ptr;
        int hr = _comImpl->EnumAdapters1(_comPtr, i, &ptr);
        Marshal.ThrowExceptionForHR(hr);
        return new DxgiAdapter1(ptr);
    }
    ...
    public DxgiSwapChain1 CreateSwapChain(DXComObject device, in DxgiSwapChainDesc desc)
    {
    ...
    /// <param name="desc">A <c>DXGI_SWAP_CHAIN_DESC</c> structure for the swap-chain description.</param>
    /// <returns>An <c>IDXGISwapChain</c> interface for the swap chain</returns>
    public DxgiSwapChain1 CreateSwapChain(nint device, in DxgiSwapChainDesc desc)
    {
        ...
        try
        {
            return new DxgiSwapChain1(DXUtils.QueryInterface(ptr, DxgiSwapChain1.DxgiSwapChain1Guid));
        }
        finally
        {
            DXUtils.Release(ptr);
    ...
    /// <returns>An adapter.</returns>
    public DxgiAdapter1 CreateSoftwareAdapter(nint module)
    {
        nint ptr;
